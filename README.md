...
Soul 记忆不是传统的长期记忆，而是**用户与智能体深度绑定的灵魂内核**。它镌刻用户的性格偏好、情感习惯、专属交互逻辑、私密情感联结，是智能体的身份本源。LLM 在对话中自主读取和写入 Soul，SQLite 持久化存储。

- **标签化与权重**：每条记忆支持自定义标签（如 `emotion:happy`、`scene:work`）和权重评分（0.0~1.0），高权重记忆优先被 LLM 读取
- **智能搜索**：支持关键词全文搜索 + 标签过滤，快速定位特定记忆
- **版本回溯**：每次修改自动保存历史版本，支持一键回滚到任意历史版本
- **遗忘机制**：模拟人脑遗忘曲线——低权重、长期未访问的记忆自动归档，访问频率高的记忆获得权重增益
- **上下文关联**：记忆可关联到具体对话消息，追溯记忆来源

Web 面板提供可视化的 Soul 管理界面，支持标签编辑、权重调节、搜索过滤，权重按绿（>0.7）/黄（>0.3）/红（<0.3）颜色指示。

### 🔄 自进化引擎——定时任务 + 自然语言 Skill

智能体具备活态自进化能力，无需人工干预即可持续迭代：

- **定时任务系统**：支持 HTTP 请求、Shell 命令、文本指令三种任务类型，间隔/每日/一次性/消息计数触发。口语化一键创建——「每天 20 点自动获取国际财经新闻，整合生成 HTML 文件保存至桌面」，让智能体越用越强大
- **任务容错与历史**：自动重试机制（可配置最大重试次数和间隔），每次执行结果持久化记录（SQLite），Web 面板可视化查看执行历史和耗时统计
- **零代码 Skill 生成**：纯自然语言描述需求，AI 自动生成完整技能配置（JSON / SKILL.md 格式），支持单功能和多子功能 Skill，兼容 OpenClaw 格式导入
- **Skill 热重载**：修改/创建 Skill 后即时生效，无需重启 Gateway，秒级完成工具注册更新
- **Skill 测试与依赖检测**：一键测试 Skill 结构完整性，自动检测 Shell 命令依赖（curl/python/node 等）是否可用，支持 dry-run 预览

### 💬 极致拟人化交互

搭配深度定制的拟人化提示词、时间感知（发送给 LLM 时自动附加本地时间戳，数据库仅存原始消息）、空闲主动发起对话、消息智能分片等能力，结合 Soul 记忆的灵魂内核，智能体拥有真人级的情感反馈逻辑，提供沉浸式、有温度的专属情感陪伴。

- **自动响应模式**：用户空闲后 LLM 自主发起对话，可配置时间区间
- **场景化主动对话**：内置早安问候（06:00-09:00）、下班关怀（17:00-19:00）、周末闲聊（周六日）三种场景，自动匹配时间段和星期，注入高权重 Soul 记忆作为上下文，实现拟真主动交互
...
- **表情包管理**：本地表情包库搜索和发送
- **自定义技能**：JSON / SKILL.md 格式，支持命令式、HTTP 式和网页抓取（Scrape）模式
...
| 页面 | 功能 |
|------|------|
| **仪表盘** | 运行状态、在线时长、通道健康度、Token 统计，首次使用自动跳转新手引导 |
| **聊天** | 三栏布局：会话列表 + 对话面板（含 Soul 记忆）+ Agent 面板，支持消息右键删除、分片显示、语音回复，消息点赞/踩反馈 |
| **技能管理** | 动态增删改查，自然语言生成，OpenClaw 格式导入，热重载即时生效，一键测试结构和依赖 |
| **定时任务** | 创建和管理定时任务，支持间隔/每日/一次性/消息计数触发，可配置重试策略，查看执行历史和耗时 |
| **联系人** | 可视化管理，支持搜索 |
| **Agent 管理** | 多 Agent 配置，AI 一键生成完整人物设定 |
| **新手引导** | 6 步交互式向导：欢迎 → 创建 Agent → 配置 LLM → 创建 Skill → 测试对话 → 探索功能，可跳过 |
...
- **登录速率限制**：登录失败指数退避延迟（1s→2s→4s→8s→16s），5 分钟窗口自动过期，成功登录清零
- **配置凭证加密**：ApiKey/Password/Token/BotToken 等敏感字段使用 Windows DPAPI 加密存储，支持明文兼容
- **LLM 错误信息汉化**：401/403/404/429/5xx 返回中文友好提示与操作指引，API 返回错误信息优先展示
- **生产构建关闭 sourcemap**：生产构建禁用 sourcemap，防止前端源码暴露
- **Soul 记忆系统全面升级**：标签化分类、权重评分（颜色指示器）、关键词搜索、访问计数，版本历史自动保存与一键回滚
- **拟真遗忘机制**：模拟人脑遗忘曲线，低权重/长期未访问记忆自动归档，高访问频率记忆获得增益
- **场景化主动对话**：内置早安/下班/周末三套场景模板，自动匹配时间与星期，注入 Soul 上下文实现拟真主动交互
- **Skill 热重载**：修改/创建 Skill 后即时生效，无需重启 Gateway
- **Skill 测试与依赖检测**：一键验证结构完整性，自动检测 Shell 命令依赖可用性，支持 dry-run 预览
- **任务容错增强**：可配置重试次数与间隔，执行历史持久化记录，Web 面板可视化查看
- **新手引导**：首次使用 6 步交互式向导，可跳过可重新打开
- **消息删除修复**：右键删除消息后即时从界面移除，不再残留错位
- 聊天页改用 HTTP POST 发消息，WebSocket 仅保留语音反馈通道，无需 AccessKey 也可正常聊天
- 时间参数注入改为运行时动态追加（仅发给 LLM，不存数据库），支持任意时区，消息记录更干净
- 修复用户消息截断 bug，复制功能正确提取纯文本（兼容图片消息 JSON 格式）
- 聊天气泡移除时间显示，界面更简洁
- 聊天 HTTP 接口支持多模态图片（URL / Base64）
- JSON Skill 新增网页抓取（Scrape）模式，通过 CSS 选择器从 HTML 页面提取结构化数据
